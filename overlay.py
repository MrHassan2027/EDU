...
class DrawingOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._strokes: list[tuple[list[QPointF], QColor, int]] = []
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton and self._current:
            self._strokes.append((list(self._current), QColor(self._color), self._width))
            self._current.clear()
            self.update()
    ...
    def _paint_stroke(painter: QPainter, points: list[QPointF], color: QColor, width: int):
        ...
        # Smooth through midpoints so strokes look fluid, not jagged
